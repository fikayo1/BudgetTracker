      ...
      animation: {
        onComplete: () => {
          ...
          if (data.length === 0) {
            ...
            ctx.fillText("No expenses for the selected Date", textX, textY);
          }
        }
      }
    }
...
const updateTable = (data) => {
  ...
  Object.keys(data).forEach((category) => {
    const { amount_spent, budget } = data[category];

    const row = document.createElement("tr");
    const categoryCell = document.createElement("td");
    categoryCell.textContent = category;
    const amountSpentCell = document.createElement("td");
    amountSpentCell.textContent = amount_spent;
...
const getChartData = (year, month) => {
  ...
  fetch(`/expense_category_summary/${year}/${month}`)
    .then((res) => res.json())
    .then((results) => {
      ...
      const [labels, data] = [
        ...
        Object.values(category_data).map((item) => item.amount_spent),
      ];
      
      console.log(data)
      renderChart(data, labels);
      updateTable(category_data);
    });
};
